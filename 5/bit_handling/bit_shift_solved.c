...
//Returns 0 if it is not enabled or 1 if it is enabled.
//NOTE: A conditional statement is false, if it produces 0
...
void writeDataToLCD(uint8_t data) {
    if (isLCDEnabled()) {
        ...
        // Set the new data
        lcd_register = lcd_register | (data << 3) & DATA_MASK;//DATA MASK NOT NEEDED
