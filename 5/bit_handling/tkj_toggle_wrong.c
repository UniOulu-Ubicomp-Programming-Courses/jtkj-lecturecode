...
// Function to toggle bits based on a given mask. Toggle bits that are 1 in the mask
void toggle(uint8_t mask) {
    
    g_register = g_register ^ mask & g_register; //THIS IS WRONG
    
    printf("After incorrect toggle: 0x%02X\n", g_register);
